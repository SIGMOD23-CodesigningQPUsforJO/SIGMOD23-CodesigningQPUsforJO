        ...
        for at in annealing_times:
            for i in range(3, 6):
                ...
                for j in range(20):
                    ...
                    best_join_order, best_join_order_costs, valid_ratio, optimal_ratio, first_opt_sample = Postprocessing.postprocess_DWave_response(response, card, pred, pred_sel, thres, optimal_join_order_costs = 0)
                    if aggregate_data:
...
def add_randomized_batch_to_csv(aggregate_data = True):
    ...
    first_opt_samples = []
    for j in range(20):
        problem_path = 'ExperimentalAnalysis/DWave/QPUPerformance/Problems/CYCLE_query/3relations'
        card, pred, pred_sel, thres = load_problem_from_disc(problem_path)
        response_path = 'ExperimentalAnalysis/DWave/QPUPerformance/Results/Data/Random/CYCLE_query/3relations/' + str(j) + "/best_embedding_response.txt"
        ...
        if aggregate_data:
            ...
            if first_opt_sample is None:
                first_opt_sample = 'None'
            save_to_csv(['Guessing', 'CYCLE', j, first_opt_sample], path, 'opt_index_results.txt')
    if aggregate_data:
        mean_first_opt_sample = None
        std_first_opt_sample = None
        if None in first_opt_samples:
            ...
            mean_first_opt_sample = np.ceil(np.mean(first_opt_samples))
            std_first_opt_sample = np.ceil(np.std(first_opt_samples))
        save_to_csv(['Guessing', 'CYCLE', mean_first_opt_sample, std_first_opt_sample], path, 'aggregated_opt_index_results.txt')                    
...
def parse_QPU_opt_sample_index(aggregate_data = False, include_guessing = False):
    ...
    graph_types = ['CLIQUE']
    if aggregate_data:
        save_to_csv(['num_relations', 'annealing_time', 'query_graph', 'mean_first_opt_index', 'std_first_opt_index'], path, 'aggregated_opt_index_results.txt')
    else:
        save_to_csv(['num_relations', 'annealing_time', 'query_graph', 'sample_index', 'first_opt_index'], path, 'opt_index_results.txt')
    for graph_type in graph_types:
        for at in annealing_times:
            for i in range(2, 5):
            #for i in range(3, 4):
                ...
                for j in range(20):
                    ...
                    if aggregate_data:
                        ...
                        if first_opt_sample is None:
                            first_opt_sample = 'None'
                        save_to_csv([i, at, graph_type, j, first_opt_sample], path, 'opt_index_results.txt')
                if aggregate_data:
                    mean_first_opt_sample = None
                    std_first_opt_sample = None
                    if None in first_opt_samples:
                        ...
                        mean_first_opt_sample = np.ceil(np.mean(first_opt_samples))
                        std_first_opt_sample = np.ceil(np.std(first_opt_samples))
                    save_to_csv([i, at, graph_type, mean_first_opt_sample, std_first_opt_sample], path, 'aggregated_opt_index_results.txt')                    
    if include_guessing:
